...
namespace JSON
{
    ...
    private:
        ...
        public:
            ...
            {
				js_parse(boost::fusion::at<Index>(object), 
    ...
    {
        void js_parse(std::string const& name, PropertyTree const& tree, ParsingOptions const& options = DEFAULT_PARSER_OPTIONS)
        {
	...
	namespace Internal {
		...
		private:
			...
			struct BaseMixin 
			{ 
				void js_parse(std::string const&,
							  PropertyTree const&,
	...
	js_parse(T& value, std::string const& name, PropertyTree const& tree, ParsingOptions const& options = DEFAULT_PARSER_OPTIONS)
	{
		value.js_parse(name, tree, options);
	}
}
